...
public class RacetrackBuidPitstop {
  ...
  public RacetrackBuidPitstop name(NameEnum name) {
    this.name = name;
    return this;
  }
  ...
  **/
  ...
  public RacetrackBuidPitstop addPitstopActionsItem(RacetrackBuidPitstopAction pitstopActionsItem) {
    if (this.pitstopActions == null) {
      this.pitstopActions = new ArrayList<>();
    }
  ...
  * @throws IOException if the JSON Element is invalid with respect to RacetrackBuidPitstop
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RacetrackBuidPitstop.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RacetrackBuidPitstop.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RacetrackBuidPitstop` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // validate the required field `pitstopActions` (array)
      for (int i = 0; i < jsonArraypitstopActions.size(); i++) {
        RacetrackBuidPitstopAction.validateJsonElement(jsonArraypitstopActions.get(i));
      };
  }
