...
public class PartnerOfferWithSessions {
  ...
  private List<PartnerOfferSession> sessions;
  ...
  * @throws IOException if the JSON Element is invalid with respect to PartnerOfferWithSessions
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PartnerOfferWithSessions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PartnerOfferWithSessions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartnerOfferWithSessions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("sessions") != null && !jsonObj.get("sessions").isJsonNull()) {
        ...
        if (jsonArraysessions != null) {
          ...
          for (int i = 0; i < jsonArraysessions.size(); i++) {
            PartnerOfferSession.validateJsonElement(jsonArraysessions.get(i));
          };
        }
      }
  }
