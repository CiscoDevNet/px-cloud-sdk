...
public class PartnerOfferAttendee {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PartnerOfferAttendee
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PartnerOfferAttendee.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PartnerOfferAttendee.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartnerOfferAttendee` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("attendeeId") != null && !jsonObj.get("attendeeId").isJsonNull()) && !jsonObj.get("attendeeId").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("attendeeUserEmail") != null && !jsonObj.get("attendeeUserEmail").isJsonNull()) && !jsonObj.get("attendeeUserEmail").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("attendeeUserFullName") != null && !jsonObj.get("attendeeUserFullName").isJsonNull()) && !jsonObj.get("attendeeUserFullName").isJsonPrimitive()) {
