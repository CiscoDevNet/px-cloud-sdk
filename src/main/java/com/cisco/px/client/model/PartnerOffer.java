...
public class PartnerOffer {
  ...
  private List<SuccessTrackMapping> mapping;
  ...
  * @throws IOException if the JSON Element is invalid with respect to PartnerOffer
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PartnerOffer.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PartnerOffer.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartnerOffer` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("customerList") != null && !jsonObj.get("customerList").isJsonNull()) {
        ...
        if (jsonArraycustomerList != null) {
          ...
          for (int i = 0; i < jsonArraycustomerList.size(); i++) {
            CustomerInfo.validateJsonElement(jsonArraycustomerList.get(i));
          };
        }
      }
      ...
      if (jsonObj.get("mapping") != null && !jsonObj.get("mapping").isJsonNull()) {
        ...
        if (jsonArraymapping != null) {
          ...
          for (int i = 0; i < jsonArraymapping.size(); i++) {
            SuccessTrackMapping.validateJsonElement(jsonArraymapping.get(i));
          };
        }
      }
      ...
      if ((jsonObj.get("userEmailId") != null && !jsonObj.get("userEmailId").isJsonNull()) && !jsonObj.get("userEmailId").isJsonPrimitive()) {
