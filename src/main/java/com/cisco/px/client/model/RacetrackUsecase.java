...
public class RacetrackUsecase {
  ...
  * @throws IOException if the JSON Element is invalid with respect to RacetrackUsecase
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RacetrackUsecase.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RacetrackUsecase.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RacetrackUsecase` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("usecaseId").isJsonPrimitive()) {
      ...
      if (!jsonObj.get("currentPitstop").isJsonPrimitive()) {
      ...
      // validate the required field `pitstops` (array)
      for (int i = 0; i < jsonArraypitstops.size(); i++) {
        RacetrackBuidPitstop.validateJsonElement(jsonArraypitstops.get(i));
      };
  }
